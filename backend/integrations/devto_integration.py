...
class DevToIntegration:
    ...
    def __init__(self):
        ...
        if not self.api_key:
            logger.warning("⚠️ DEVTO_API_KEY not found. Running in MOCK MODE.")
            self.mock_mode = True
        else:
    ...
    def post_article(self, title, content_markdown, tags=[], cover_image=None, published=False, canonical_url=None):
        ...
        try:
            ...
            data = response.json()
            logger.info(f"✅ Posted to DEV.to: {data.get('url')}")
            ...
            logger.error(f"❌ DEV.to Post Failed: {e}")
    ...
    def _mock_post(self, title, content, tags, cover_image, published):
        ...
        logger.info(f"🤖 [MOCK] Posted to DEV.to")
