"""
Phase 3（Uncle Sam / 佐世保バーガー）の4テーマでnote記事を生成するスクリプト
"""
import os
import sys
import json
...
def generate_article(theme: dict, project_day: int = 360) -> dict:
    ...
    prompt = (
        ...
        "- この記事の舞台は佐世保バーガーの店（従業員7名→4名に減った）。バーテンダー・商品開発・人材教育・店舗運営・イベント企画に従事していた\n"
        "- ファストフードの接客に加え、バーの「人と深く繋がる接客スタイル」を取り入れてリピート率を向上させた経験がある\n"
        "- お客様とスタッフでイベントを企画し、新規顧客を獲得した実績がある\n"
        ...
        "- 「佐世保バーガーの店」「ハンバーガー店」「バーガーショップ」など一般的表現に置き換える\n"
        "- Uncle Samは使わない\n"
        "- 地域を特定できる情報は書かない\n\n"
        "【半ノンフィクション方針】\n"
        ...
        "  例：「バーガー屋にバーの接客を持ち込んだ。正直、最初は笑われた。」\n"
        "パターン2（疑問型）：「〜を〜したら、何が起こるのか」\n"
        "  例：「ファストフードの接客を全部変えたら、何が起こるのか」\n"
        "パターン3（結論先出し）：「〜を諦めた。理由は〜だった」\n"
        "  例：「マニュアル接客を諦めた。理由は、常連が一人も増えなかったからだ」\n"
        "⚠️ 禁止タイトル：「〜のすすめ」「〜完全ガイド」「〜5選」などのハウツー系\n\n"
        "【記事構成（1500〜2500文字 ※厳守）】\n"
        "1. タイトル：上記パターンのどれかを使う（Day番号不要）\n"
        "2. 書き出し（3〜5文）：バーガーショップ時代の現場の一幕から入る\n"
        "3. 本文（1200〜2000文字）：上記の構成に従って。読者が「自分のことだ」と感じる一次体験を軸に書く。※必ず1500文字以上書くこと※\n"
        "4. 締め（さりげないCTA）：\n"
        "   売り込まず、存在を伝える程度に。\n"
        ...
        "JSONのみ返答してください。本文は必ず1500文字以上にしてください。"
    )
    ...
    system_msg = (
        ...
        "必須：1500〜2500文字。JSONのみ返す。短い記事は絶対にNG。"
    )
...
if __name__ == "__main__":
    print("=" * 70)
    print("Phase 3: Uncle Sam（佐世保バーガー）— 4テーマで記事生成")
    print("=" * 70)

    results = []
    for i, theme in enumerate(PHASE3_THEMES):
        print(f"\n--- テーマ {i+1}/4: {theme['concept']} ---")
        print(f"フック: {theme['hook']}")
        print("生成中...")
        try:
            article = generate_article(theme, project_day=370 + i)
    ...
    output_path = os.path.join(os.path.dirname(__file__), "..", "data", "phase3_drafts.json")
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(results, f, ensure_ascii=False, indent=2)

    print(f"\n{'=' * 70}")
    print(f"✅ 4記事の生成完了！保存先: {output_path}")
    print(f"{'=' * 70}")

    for i, article in enumerate(results):
        if "error" in article:
            print(f"\n【記事 {i+1}】❌ エラー: {article['error']}")
            continue
        print(f"\n{'━' * 70}")
