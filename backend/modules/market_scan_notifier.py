"""
MarketScanNotifier — スキャン結果を Slack / Telegram / Notion に通知する。
...
class MarketScanNotifier:
    ...
    # ── Notion (BlogScheduler キューへ追加) ────────────────────────────────

    def notify_notion(self, scan_result: dict[str, Any]) -> bool:
        """
        スキャン結果のトップ機会を Notion Content Pool DB に追加する。
        BlogScheduler が次のサイクルで自動的に記事生成に使う。
        """
        ...
        if not db_id or not api_key:
            logger.warning("[NOTIFIER] NOTION_CONTENT_POOL_DB_ID or NOTION_API_KEY missing. Skipping.")
            return False

        opportunities = scan_result.get("opportunities", [])
        if not opportunities:
            logger.info("[NOTIFIER] No opportunities to add to Notion.")
            return False
        ...
        try:
            ...
            for opp in opportunities[:BLOG_QUEUE_TOP_N]:
                ...
                if self.dry_run:
                    logger.info(f"[NOTIFIER][DRY_RUN] Notion: Would add '{topic}' to Content Pool.")
                    added += 1
                    continue

                result = notion.add_to_database(db_id, properties, content=content)
                if result:
                    logger.info(f"[NOTIFIER] ✅ Notion: Added '{topic}' to Content Pool.")
                    added += 1
                else:
                    logger.warning(f"[NOTIFIER] Notion: Failed to add '{topic}'.")
    ...
    def notify_all(self, scan_result: dict[str, Any]) -> dict[str, bool]:
        """
        Slack / Telegram / Notion にまとめて通知する。

        Returns:
            {"slack": bool, "telegram": bool, "notion": bool}
        """
    ...
    def _build_text(self, scan_result: dict[str, Any], platform: str = "slack") -> str:
        """Slack/Telegram共通のメッセージ文字列を生成する。"""
        opportunities = scan_result.get("opportunities", [])
