...
class LangGraphOrchestrator:
    def __init__(self):
        ...
        self.obsidian_agent = safe_init("ObsidianConnector", lambda: ObsidianConnector())
    ...
    def run(self, user_input: Union[str, Dict], model_name: str = "gemini"):
        ...
        # Determine if we were passed a full state dict or a single string message
        if isinstance(user_input, dict):
            # If passed a full AgentState-like dict, use it as base
            inputs = {
                "messages": user_input.get("messages", []),
                "model_name": user_input.get("model_name", model_name),
                "plan": user_input.get("plan", []),
                "current_step_index": user_input.get("current_step_index", 0),
                "context": {**self._persistent_context, **user_input.get("context", {})},
                "final_response": user_input.get("final_response", "")
            }
        else:
            # Traditional string message path
            user_message = str(user_input)
            
            # Load recent history to provide context
            history = []
            if self.memory_agent:
                 session_id = self._persistent_context.get('session_id', 'global_session')
                 history = self.memory_agent.get_short_term(limit=5, session_id=session_id)
        ...
        try:
            ...
            # Update persistent context for next turn
            self._persistent_context = result.get('context', {})
            return result
        except Exception as e:
            logger.error(f"Graph execution failed: {e}", exc_info=True)
            return {"status": "error", "message": str(e), "final_response": f"Error during graph execution: {str(e)}"}
    ...
    def plan_node(self, state: AgentState):
        ...
        with open(debug_file, "a", encoding="utf-8") as f: f.write(f"CHECKED_TOP: {req} (is_pronoun={is_pronoun_query})\n")
        ...
        # GENERAL SEARCH HEURISTIC (New Fix)
             ...
             if any(k in req for k in ["research", "調べて", "調べ", "論文", "net", "ネット", "search"]):
