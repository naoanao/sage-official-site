...
> 調査・更新日: 2026-06-19 | Support AI実装およびGoogleディスプレイ広告参考設計の追加

---

## 🔥 最新更新（2026-06-19）— 問い合わせAI（Support AI）の実装およびディスプレイ広告参考設計

**問い合わせ対応自動化システム（Support AI）の実装**
- **セキュアWebhookの導入**: Node.js標準の `crypto` を利用した HMAC-SHA256 署名検証（Svix互換）を `app/api/webhook/inbound-email/route.ts` に実装。`RESEND_SIGNING_SECRET` に基づきリクエストの正当性を完全にチェック可能にし、悪意あるスパムやリプレイ攻撃を防ぐ強固な防壁を構築。
- **LLM自動フォールバックチェーン**: API制限による稼働停止を防ぐため、`Gemini (429) ➔ DeepSeek (402) ➔ Groq (Llama 3.3 70B)` の自動例外切り替え（フォールバック）ロジックを実装。どれか一つのプロバイダーに障害や制限が起きても、確実に問い合わせの自動分類と下書き作成を完遂可能。
- **メール承認返信（GET/approve-email）**: 個人宛Gmailに届くリンクを1タップするだけで、自動で問い合わせユーザーへメールを返信し、Supabase上のチケットステータスを `PENDING` から `REPLIED` に更新。送信成功時には美しいHTML送信完了画面を提示。
- **E2E自動テストの確立 (`scratch/run_support_ai_e2e.js`)**: 署名の自動生成・Webhook POST・Supabase検証・承認API GET・ステータス更新・DBクリーンアップまでの一連のE2Eライフサイクルを検証するテストスクリプトを作成。全テストの正常通過を確認し、本番環境（Vercel）へデプロイ完了。

**Googleディスプレイ広告参考設計とTSV出力最適化**
- **代替え可能なディスプレイ広告の参考設計**: 今後のGrowl広告機能の汎用テンプレートとして、ファンケル青汁を題材にした「興味・関心」「比較検討」「CV獲得（リターゲティング）」の3キャンペーン構成からなる設計モデルを策定。他商材へ容易に代替え・流用可能な構成になっています。
- **TSVフォーマットの汎用調整**: Googleスプレッドシート（gid=307652644）への貼り付け時に列ズレを起こさない空列（Column G）考慮のタブ幅調整や、他商材適用時にも過度なAND条件を防ぐためにリターゲティング・比較検討キャンペーンからコンテンツカテゴリー設定を除外する汎用的な最適化ルールを確立。
