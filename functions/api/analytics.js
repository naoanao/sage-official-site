...
export async function onRequestGet({ request, env }) {
  ...
  try {
    const [
      ...
      // 1. Total modal views
      db.prepare(`SELECT COUNT(*) as n FROM funnel_events WHERE event='modal_view'`).first(),

      // 2. Total payment clicks
      db.prepare(`SELECT COUNT(*) as n FROM funnel_events WHERE event='payment_click'`).first(),

      // 3. All subscribers with created_at
      db.prepare(`SELECT email, created_at, plan FROM subscribers WHERE status='active' ORDER BY created_at DESC`).all(),
    ...
    const trendRaw = await db.prepare(`
