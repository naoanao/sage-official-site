...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    fetch(`/api/tiktok/status?device_id=${encodeURIComponent(deviceId)}`)
      .then((r) => r.json())
      .then((d) => {
        setTiktokConnected(!!d.connected);
        if (d.connected) {
          setTiktokUser({ display_name: d.display_name, avatar_url: d.avatar_url });
        }
      })
    ...
    <main className="min-h-screen bg-gray-50 px-4 py-10">
      ...
      <div className="max-w-lg mx-auto">
        ...
        {tiktokConnected === false && (
          <div className="mt-6 rounded-2xl overflow-hidden shadow-md" style={{ background: "linear-gradient(135deg, #010101 0%, #2d2d2d 100%)" }}>
            <div className="px-5 py-4 flex items-start gap-3">
              <div className="text-3xl shrink-0">🎵</div>
              <div className="flex-1">
                <p className="font-bold text-white text-sm mb-0.5">
                  {isEn ? "Connect TikTok" : "TikTokと連携する"}
                </p>
                <p className="text-xs leading-relaxed mb-3" style={{ color: "rgba(255,255,255,0.75)" }}>
                  {isEn
                    ? "Connect your TikTok account to auto-post videos generated by Growl."
                    : "TikTokアカウントを連携して、Growlが生成した動画を自動投稿しましょう。"}
                </p>
                <button
                  type="button"
                  onClick={() => {
                    const deviceId = localStorage.getItem("growl_device_id") || "global";
                    window.location.href = `/api/auth/tiktok?device_id=${encodeURIComponent(deviceId)}`;
                  }}
                  className="bg-white font-bold text-sm px-4 py-2 rounded-xl transition-all active:scale-95"
                  style={{ color: "#010101" }}
                >
                  {isEn ? "Connect TikTok →" : "TikTokを連携する →"}
                </button>
              </div>
            </div>
          </div>
        ...
        {tiktokConnected === true && (
          <div className="mt-6 rounded-2xl bg-white border border-gray-100 shadow-sm px-5 py-4 flex items-center gap-3">
            {tiktokUser?.avatar_url ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={tiktokUser.avatar_url} alt="TikTok" className="w-9 h-9 rounded-full object-cover shrink-0" />
            ) : (
              <div className="w-9 h-9 rounded-full bg-gray-100 flex items-center justify-center text-lg shrink-0">🎵</div>
            )}
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-widest">TikTok</p>
              <p className="text-sm font-bold text-gray-800 truncate">
                {tiktokUser?.display_name || (isEn ? "Connected" : "連携済み")}
              </p>
            </div>
            <span className="text-xs font-medium text-green-500 bg-green-50 px-2 py-0.5 rounded-full shrink-0">
              {isEn ? "Connected" : "連携済み"}
            </span>
          </div>
        )}
