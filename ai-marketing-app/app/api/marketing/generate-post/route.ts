...
【ハルシネーション禁止ルール（最重要・絶対厳守）】
- 入力情報（商品・サービス: ${product}）に記載のない商品名・メニュー名・新機能・特典・キャンペーン名・割引率・具体的な金額・数値を絶対に作り出さないこと
- 「○○が追加されました」「○○%OFF」「期間限定△△円」など、入力に存在しない事実を補完・推測・捏造して投稿文に含めることは厳禁
- 存在しないサービス・商品・イベントを架空で追加しないこと
- 入力された情報の範囲内でのみ、言葉のトーンや表現を工夫すること
