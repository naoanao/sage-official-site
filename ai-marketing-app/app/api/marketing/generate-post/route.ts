...
export async function POST(req: NextRequest) {
  try {
    ...
    const prompt = `あなたは日本の小規模事業主専門のSNSコピーライターです。以下の情報をもとに「今日そのままコピー&ペーストしてSNSに投稿できる文章」を3本生成してください。
...
【分析フレームワーク】${framework}
【マーケティングインサイト】${insight}

【生成ルール】
- Markdown記号（**、##、___、\`など）は一切使わない
- 各投稿は実際にそのままコピーして投稿できる完成した文章
- 3本それぞれ異なる切り口（集客・信頼構築・お得情報など）で作る
- 架空のURL（example.comなど）は絶対に使わない
- インサイトから得た知見を実際の投稿内容に反映させる
- 投稿文は読んだ人が「行ってみたい」「試してみたい」と感じるものにする
- ${postHint}
