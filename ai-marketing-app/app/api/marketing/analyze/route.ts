...
const FRAMEWORK_PROMPTS: Record<string, (c: CompanyInfo) => string> = {
  ...
  "actions": [
    "【絶対条件：広告費・外部依頼・イベント企画・インフルエンサー依頼は禁止。スマホのみ・今週中・30分以内・無料で完結するアクションのみ】具体的なツール名・ハッシュタグ・文章例・実行後に確認するKPIをセットで書くこと（例：[該当SNS]で「#ターゲットの悩み」を付けた投稿を作成→24時間後のインプレッションを記録する。※これは例です。必ず『入力されたビジネス』に最適なチャネルと内容に書き換えること。BtoBやSaaSの場合はカフェや実店舗向けの提案を絶対にしないこと）",
    "【同条件・別チャネル】2つ目のアクション（例：過去の顧客やユーザー3名に利用の感想を求めるメッセージを送り、1週間で返信数を確認する。※実店舗でない場合はGoogleマップや店舗用ツールの提案は避けること）",
    "【同条件・競合分析系】3つ目のアクション（例：競合上位3社の発信内容で反応が良い投稿を3〜5件分析し、自社との差異をメモして次の発信テーマに反映する）"
  ],
  "strategy_summary": {
...
${jsonTemplate}`;
  ...
  "items": {
    "顧客の動き（市場）": ["このターゲット顧客が今夜布団の中で考えている悩み・不安の具体的な姿1", "その悩みが解決されたとき顧客の人生がどう変わるか（より良い未来）2", "購買をためらわせている最大の心理的障壁と、それを乗り越えるきっかけ3"],
    "ライバル店の動き（競合）": ["競合が力を入れていることと、それによって生まれる顧客の不満・物足りなさ1", "競合が気づいていない・対応できていない顧客の深い欲求（この事業者が狙えるギャップ）2", "このギャップを埋めたとき、顧客がこの事業者に感じる唯一無二の価値3"],
    "自社の提供価値（Company）": ["この事業者だけが顧客に提供できる具体的な人生の変化・体験1", "競合に絶対に真似できない理由（技術・想い・歴史・関係性など）2", "正直に認めるべき課題と、それでも顧客に選ばれるための誠実な一手3"]
  },
  "promotion_gap": {
...
${jsonTemplate}`;
  ...
  "items": {
    "自店の強み": ["この強みによって顧客のどんな悩みが消え、どんな未来が生まれるか1", "競合には絶対に真似できない理由（技術・想い・歴史・仕組みなど）2", "この強みをまだ知らない顧客に伝わっていない理由と、伝えるべき言葉3"],
    "伸びしろ（今後の課題）": ["顧客が購買をためらう最大の原因となっている自社の弱点1", "この弱点を放置し続けたときに失われる顧客と売上2", "弱点を逆手に取って誠実さに変える方法3"],
    "機会（市場の変化が生むチャンス）": ["2026年のターゲット顧客の生活・価値観の変化でこのビジネスに生まれた具体的なチャンス1", "競合がまだ動いていない、今すぐ先手を打てる市場の空白2", "このチャンスを掴むためにこの事業者だけができること3"],
    "脅威（乗り越えるべき壁）": ["このビジネスを最も脅かしている外部環境の変化と、その影響の具体像1", "脅威を無視し続けた場合の最悪シナリオ2", "この脅威を逆にチャンスに変える発想の転換3"]
  },
...
${jsonTemplate}`;
  ...
  "items": {
    "セグメンテーション（欲求で分ける）": ["このビジネスの顧客を「抱えている悩みの深さ」で分けた最も有望なセグメント1", "「理想の未来のイメージ」で分けた購買意欲の高いセグメント2", "今すぐ狙えるのに見落とされているニッチなセグメント3"],
    "ターゲティング（最も変化を届けられる顧客）": ["今すぐ集中すべき最優先セグメントと、その理由（悩みの深さ・購買力・口コミ影響力）1", "このセグメントが今まさに抱えている具体的な悩みの姿2", "このセグメントに選ばれるために他社との戦い方3"],
    "ポジショニング（顧客の心の中での場所）": ["競合が占めていない、この事業者だけが主張できる独自のポジション1", "そのポジションを一言で表すキャッチコピーの方向性（例文付き）2", "そのポジションを証明する最も説得力のある事実・証拠3"]
  },
...
${jsonTemplate}`;
  ...
  "items": {
    "商品（Product）—顧客が本当に買っているもの": ["顧客がこの商品を通じて手に入れたい「感情・体験・人生の変化」（機能説明は禁止）1", "競合には絶対に真似できないこだわりと、それが顧客の悩みをどう解決するか2", "この商品を使った後に顧客が周りに言いたくなる一言3"],
    "価格（Price）—価値と価格の橋渡し": ["${c.price ? `${c.price}という価格設定で顧客に「高い」と感じさせる心理的原因と解消策` : "この商品の価値に見合った最適な価格設定と根拠"}1", "価格への抵抗感を「投資対効果・LTV」に変える言葉の作り方（入口価格と継続価格を分けて設計する視点を含む）2", "価格を上げずに知覚価値を高める最も効果的な方法3"],
    "販売チャネル（Place）—顧客が買いやすい場所と導線": ["このターゲット顧客が最も自然に出会える接点チャネル1", "「知る→興味→買う」の最短導線設計と離脱しやすいポイント2", "今すぐ改善できる購入障壁の取り除き方3"],
    "プロモーション（Promotion）—顧客の心を動かすメッセージ": ["今すぐ始められる最優先集客施策と、それが効く理由（SNS・MEO・チラシなど）1", "顧客の「今夜の悩み」から始まるコンテンツ戦略（認知段階・比較検討段階・購入後段階で使い分ける）2", "口コミ・紹介が自然に起きる仕掛けの具体案（ダブルファネルの継続・紹介・発信フェーズの設計）3"]
  },
  "ad_strategy": {
...
${jsonTemplate}`;
  ...
  "items": {
    "価値（Value）—顧客の人生に与える価値": ["この強みによって顧客のどんな悩みが消え、どんな人生の変化が生まれるか1", "その価値が売上・利益に繋がる具体的なメカニズム2", "競合と比べてこの価値がどれだけ大きいかを示す具体的な証拠3"],
    "希少性（Rarity）—他社にない強み": ["競合が持っていない技術・ノウハウ・人材・関係性・歴史1", "この希少性が顧客に「ここしかない」と感じさせる理由2", "この希少性をまだ顧客に伝えられていない理由と伝え方3"],
    "模倣困難性（Imitability）—真似されない理由": ["競合がコピーしようとしても越えられない参入障壁の具体的な内容1", "時間・コスト・文化・信頼の蓄積で真似できない要素2", "この模倣困難性をブランドストーリーとして顧客に伝える言葉3"],
    "組織活用（Organization）—強みを活かす仕組み": ["この強みを最大限に顧客に届けられている仕組みと体制1", "強みを活かしきれていない最大の機会損失2", "今すぐ改善できる最も費用対効果の高い組織・仕組みの一手3"]
  },
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Markdownコードブロックの除去
    const jsonStr = match[0].replace(/^```json\s*/, "").replace(/```$/, "").trim();
