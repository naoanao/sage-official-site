...
const PHASES = [
    ...
    { id: 4, label: 'Publish', icon: '🚀', desc: 'Post & publish' },
];

const PhaseStepperBar = ({ currentPhase, topic, onPhaseClick }) => {
    return (
        <div className="sticky top-0 z-20 bg-white/90 backdrop-blur-md border-b border-[var(--c-border)] px-8 py-3">
                ...
                {PHASES.map((phase, i) => {
                    ...
                    return (
                        <React.Fragment key={phase.id}>
                            ...
                            {i < PHASES.length - 1 && (
                                <span className={`text-sm ${currentPhase > phase.id ? 'text-emerald-600' : 'text-[var(--c-subtle)]'}`}>→</span>
                            )}
                        </React.Fragment>
                    );
                })}
                {topic && (
                    <span className="ml-auto text-xs text-[var(--c-subtle)] truncate max-w-xs">
                        💬 {topic}
                    </span>
                )}
            </div>
